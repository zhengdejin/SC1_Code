...
static void mt_cpufreq_power_calculation(int index, int ncpu)
{
    ...
    pr_info("mt_cpu_power[%d]: cpufreq_ncpu = %d, cpufreq_khz = %d, cpufreq_power = %d\n", (index * possiblecpu + ncpu), 
...
int mt_cpufreq_state_set(int enabled)
{
    if (enabled)
    {
        if (!mt_cpufreq_pause)
        {
            pr_info("cpufreq already enabled\n");
            return 0;
        }
        ...
        **************/
        ...
        pr_info("enable DVFS: g_dvfs_disable_count = %d\n", g_dvfs_disable_count);
        ...
        ************************************************/
        ...
        {
            pr_info("someone still disable cpufreq, cannot enable it\n");
        }
    }
    else
    {
        ...
        ***************/
        g_dvfs_disable_count++;
        pr_info("disable DVFS: g_dvfs_disable_count = %d\n", g_dvfs_disable_count);

        if (mt_cpufreq_pause)
        {
            pr_info("cpufreq already disabled\n");
            return 0;
        }
...
void mt_cpufreq_disable(unsigned int type, bool disabled)
{
	...
	if(type == 0)
	{
	    ...
	    if(mt_cpufreq_disabled_by_sdio_autoK == true)
	    {
		    ...
		    pr_info("mt_cpufreq_disable, limited freq. at %d\n", DVFS_F2);

		    cpufreq_cpu_put(policy);
	    }
		pr_info("mt_cpufreq_disable, mt_cpufreq_disabled_by_sdio_autoK = %d\n", mt_cpufreq_disabled_by_sdio_autoK);
	}
	else if(type == 1)
	{
...
void mt_cpufreq_volt_set_by_sdio(unsigned int volt)
{
	...
	pr_info("mt_cpufreq_volt_set_by_sdio, reg = %d\n", reg);
	...
	{
		pr_info("mt_cpufreq_volt_set_by_sdio, could not set volt now,  mt_cpufreq_disabled_by_sdio_autoK = %d\n", mt_cpufreq_disabled_by_sdio_autoK);
	}
#endif
}
...
void mt_cpufreq_thermal_protect(unsigned int limited_power)
{
    ...
    if (limited_power == 0)
    {
        ...
        pr_info("thermal limit g_limited_max_freq = %d, g_limited_max_ncpu = %d\n", g_limited_max_freq, g_limited_max_ncpu);
    }
    else
    {
        while (ncpu)
        ...
        if (!found)
        {
            pr_info("Not found suitable DVFS OPP, limit to lowest OPP!\n");
        ...
        if (num_online_cpus() > g_limited_max_ncpu)
        {
            ...
            {
                pr_info("turn off CPU%d due to thermal protection\n", (i - 1));
                cpu_down((i - 1));
            }
        }
...
static ssize_t mt_cpufreq_state_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    ...
    if (sscanf(buffer, "%d", &enabled) == 1)
    {
        ...
        {
            pr_info("bad argument!! argument should be \"1\" or \"0\"\n");
        }
    }
    else
    {
...
static ssize_t mt_cpufreq_limited_power_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    ...
    {
        pr_info("bad argument!! please provide the maximum limited power\n");
    }
...
static ssize_t mt_cpufreq_limited_load_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    ...
    {
        pr_info("bad argument!! please provide the limited load\n");
    }
...
static int mt_cpufreq_pdrv_probe(struct platform_device *pdev)
{
    ...
    *************************************************/
    ...
    pr_info("mediatek cpufreq initialized\n");
...
static int __init mt_cpufreq_pdrv_init(void)
{
    ...
    {
        pr_err("cpufreq driver registration done\n");
        pr_err("g_cpufreq_get_ptp_level = %d\n", g_cpufreq_get_ptp_level);
        return 0;
    }
}
