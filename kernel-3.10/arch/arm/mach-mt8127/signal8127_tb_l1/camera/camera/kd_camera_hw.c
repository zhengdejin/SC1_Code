...
{
	u32 pinSetIdx = 0, i;	//default main sensor
...
#define IDX_PS_OFF  3
	...
	printk("abc kd_sensor pinSetIdx=%d \n", pinSetIdx);
...
*/
		...
		if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_HI551_MIPI_RAW, currSensorName))) {
		    ...
		    if (TRUE != hwPowerOn(CAMERA_POWER_VCAM_A2, VOL_2800, mode_name)) {
				PK_DBG("[CAMERA SENSOR] Fail to enable AF_VCC power\n");
				//return -EIO;
		...
		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_BACK_S5K5E2YA_MIPI_RAW, currSensorName))) {
			if (TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1200, mode_name)) {
				PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
			}
			mdelay(5);
		...
		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_FRONT_S5K5E2YA_MIPI_RAW,currSensorName))) {
			if (TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1200, mode_name)) {
				PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
			}
			mdelay(5);
		...
		if (pinSetIdx == 0) {
			if (GPIO_CAMERA_INVALID != pinSet[1][IDX_PS_CMRST]) {
				...
				if(mt_set_gpio_mode(pinSet[0][IDX_PS_CMRST],pinSet[1][IDX_PS_CMRST+IDX_PS_MODE])){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n");}
				if(mt_set_gpio_mode(pinSet[0][IDX_PS_CMPDN],pinSet[1][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! \n");}
				if(mt_set_gpio_dir(pinSet[0][IDX_PS_CMRST],GPIO_DIR_OUT)){PK_DBG("[CAMERA SENSOR] set gpio dir failed!! \n");}
				if(mt_set_gpio_dir(pinSet[0][IDX_PS_CMPDN],GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! \n");}
				if(mt_set_gpio_out(pinSet[0][IDX_PS_CMRST],pinSet[1][IDX_PS_CMRST+IDX_PS_OFF])){PK_DBG("[CAMERA SENSOR] set gpio failed!! \n");} //low == reset sensor
				if(mt_set_gpio_out(pinSet[0][IDX_PS_CMPDN],pinSet[1][IDX_PS_CMPDN+IDX_PS_OFF])){PK_DBG("[CAMERA LENS] set gpio failed!! \n");} //high == power down lens module
				// wait power to be stable 
		   }
		}
    }
		...
		if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_HI551_MIPI_RAW, currSensorName))) {
			if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_A2,mode_name)) {
				PK_DBG("[CAMERA SENSOR] Fail to OFF AF_VCC power\n");
			}
			mdelay(1);
		}
		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV5670_MIPI_RAW, currSensorName))) {
