...
{
    pr_info("%s AudDrv_btcvsd_open do nothing inode:%p, file:%p \n",__func__, inode, fp);
    return 0;
}
...
static ssize_t AudDrv_btcvsd_read(struct file *fp,  char __user *data, size_t count, loff_t *offset)
{
    ...
    while (count)
    {
        ...
        else //copy twice
        {
            ...
            {
                pr_err("%s AudDrv_btcvsd_read Fail 3 copy to user Read_Data_Ptr:%p, pRX->PacketBuf:%p, BTSCORX_ReadIdx_tmp:0x%x, read_size:%x",__func__,Read_Data_Ptr, btsco.pRX->PacketBuf, BTSCORX_ReadIdx_tmp, read_size);
                if (read_count == 0)
                {
                    return -1;
                }
