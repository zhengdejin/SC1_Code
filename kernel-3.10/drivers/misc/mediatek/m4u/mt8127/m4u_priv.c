...
#define M4U_ASSERT(x) if(!(x)){pr_err("M4Uassert fail, file:%s, line:%d", __FILE__, __LINE__);}
...
static int m4u_probe(struct platform_device *pdev)
{
  ...
  //  MMP_Event M4U_Event;
    ...
    //Set IRQ
    if(request_irq(M4U_MMU0_IRQ_ID , MTK_M4U_isr, IRQF_TRIGGER_LOW, M4U_DEVNAME , NULL))
    {
        M4UERR("request M4U0 IRQ line failed\n");
        return -ENODEV;
    }

    if(request_irq(M4U_MMU1_IRQ_ID , MTK_M4U_isr, IRQF_TRIGGER_LOW, M4U_DEVNAME , NULL))
    {
        M4UERR("request M4U1 IRQ line failed\n");
        return -ENODEV;
    }

    if(request_irq(M4U_MMU_L2_IRQ_ID , MTK_M4U_L2_isr, IRQF_TRIGGER_LOW, M4U_DEVNAME , NULL))
    {
        M4UERR("request M4U2 IRQ line failed\n");
        return -ENODEV;
    }
    disable_irq(M4U_MMU_L2_IRQ_ID);

    if(request_irq(M4U_MMU_L2_SEC_IRQ_ID , MTK_M4U_L2_isr, IRQF_TRIGGER_LOW, M4U_DEVNAME , NULL))
    {
        M4UERR("request M4U2 IRQ line failed\n");
        return -ENODEV;
    }
    disable_irq(M4U_MMU_L2_SEC_IRQ_ID);
...
static int m4u_remove(struct platform_device *pdev)
{
    ...
    //Release IRQ
    free_irq(M4U_MMU0_IRQ_ID , NULL);
    free_irq(M4U_MMU1_IRQ_ID , NULL);
    free_irq(M4U_MMU_L2_IRQ_ID , NULL);
...
int __m4u_alloc_mva(mva_info_t *pMvaInfo, struct sg_table *sg_table)
{
    ...
    //for m4u bug: we need our entry_start and entry_end align at 128 bits (pfh tlb is 128bits align)
...
int m4u_invalid_seq_range_by_mva(int m4u_index, unsigned int MVAStart, unsigned int MVAEnd)
{
    ...
    //trig invalidate register, chip invalid is moved to M4U global register area
    ...
    else if(BufAddr<PAGE_OFFSET)  // from user space
    {
        ...
        else
        {
            ...
            if((vma->vm_flags) & VM_PFNMAP)
            {
                ...
                {
                    M4UMSG("error: page_num=%d,module=%s, va=0x%x, size=0x%x, vm_flag=0x%lx\n",
                ...
                M4UINFO("alloc_mva VM_PFNMAP module=%s, va=0x%x, size=0x%x, vm_flag=0x%lx\n",
...
static int m4u_pm_restore_noirq(struct device *device)
{
    ...
    // m4u related irqs
    mt_irq_set_sens(M4U_MMU0_IRQ_ID, MT_LEVEL_SENSITIVE);
	mt_irq_set_polarity(M4U_MMU0_IRQ_ID, MT_POLARITY_LOW);

    mt_irq_set_sens(M4U_MMU1_IRQ_ID, MT_LEVEL_SENSITIVE);
	mt_irq_set_polarity(M4U_MMU1_IRQ_ID, MT_POLARITY_LOW);

    mt_irq_set_sens(M4U_MMU_L2_IRQ_ID, MT_LEVEL_SENSITIVE);
	mt_irq_set_polarity(M4U_MMU_L2_IRQ_ID, MT_POLARITY_LOW);

    mt_irq_set_sens(M4U_MMU_L2_SEC_IRQ_ID, MT_LEVEL_SENSITIVE);
	mt_irq_set_polarity(M4U_MMU_L2_SEC_IRQ_ID, MT_POLARITY_LOW);
