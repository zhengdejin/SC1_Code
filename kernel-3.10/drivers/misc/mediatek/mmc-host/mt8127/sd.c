...
static void msdc_tasklet_testmode(unsigned long arg)
{
    ...
    pr_debug( "SDIO_TEST_MODE" "Enter test mode tasklet\n");
    pr_debug( "SDIO_TEST_MODE" "Tasklet, host = %x\n", host);
    pr_debug( "SDIO_TEST_MODE" "Tasklet, host->testmode_timer = %x, host->testmode_tasklet = %x\n", &host->testmode_timer, &host->testmode_tasklet);
	...
	pr_debug( "SDIO_TEST_MODE" "Tasklet, cur_temperature = %d, host->pre_temper = %d\n", cur_temperature, host->pre_temper);
...
static unsigned int msdc_online_tuning(struct msdc_host   *host, unsigned fn, unsigned addr)
{
    ...
    pr_debug( "SDIO_TEST_MODE" "Enter msdc_online_tuning \n");
    
//    pr_debug( "SDIO_TEST_MODE" "[online_tuning] mrq = %x, cmd = %x, data = %x, id = %d\n", host->mrq, host->cmd, host->data, host->id);
    
    /* Get current delay settings */
    sdr_get_field(MSDC_PAD_TUNE, MSDC_PAD_TUNE_CMDRDLY, cmdrdly);	// Get cmd delay
    pr_debug( "SDIO_TEST_MODE" "cmd delay = %d\n", cmdrdly);
    
    rxdly0 = sdr_read32(MSDC_DAT_RDDLY0);
    pr_debug( "SDIO_TEST_MODE" "DAT read delay line 0 = %x\n", rxdly0);
    ...
    pr_debug( "SDIO_TEST_MODE" "gear window size = %d\n", gear_window_size);
...
_retry:
    ...
    pr_debug( "SDIO_TEST_MODE" "enter command start\n");
    if (msdc_command_start(host, &cmd, 0, CMD_TIMEOUT) != 0) {
        host->autocmd = autocmd;
        pr_debug( "SDIO_TEST_MODE" "command start fail, err = %d\n", cmd.error);
        return cmd.error;
    }
    ...
    pr_debug( "SDIO_TEST_MODE" "CRCStatus_CMD = %d, gear_shift = %d\n", CRCStatus_CMD, gear_shift);
    if(gear_shift) {
        /* Reset delay gear */
    ...
    if(retry) {
        pr_debug( "SDIO_TEST_MODE" "goto _retry (Read) \n");
        goto _retry;
    }
    ...
    if(retry) {
        pr_debug( "SDIO_TEST_MODE" "goto _retry (Write) \n");
        goto _retry;
    }
...
//                pr_debug( "SDIO_TEST_MODE" "Trigger Timer[B], expires = %d, timeout = %d\n",host->testmode_timer.expires, host->testmode_timeout);
//                pr_debug( "SDIO_TEST_MODE" "Timer Function = %x\n",host->testmode_timer.function);
//                pr_debug( "SDIO_TEST_MODE" "Do request, host = %x, host->mrq = %x, host->id = %d\n", host, host->mrq, host->id);
//                pr_debug( "SDIO_TEST_MODE" "Do request, host->mmc = %x\n", host->mmc);
//                if(host->mmc) {
//                    pr_debug( "SDIO_TEST_MODE" "Do request, host->mmc->card = %x\n", host->mmc->card);
//                    if(host->mmc->card) {
//                        pr_debug( "SDIO_TEST_MODE" "Do request, host->mmc->card->type = %d\n", host->mmc->card->type);
//                    }
//                }
//                pr_debug( "SDIO_TEST_MODE" "Do request, cmd->opcode = %d, cmd->arg = %x, cmd->flags = %x\n", cmd->opcode, cmd->arg, cmd->flags);
//                if(data)
//                    pr_debug( "SDIO_TEST_MODE" "Do request, data->sg->dma_address = %x, data->sg->offset = %x, data->sg->page_link = %x\n", data->sg->dma_address, data->sg->offset, data->sg->page_link);
//                pr_debug( "SDIO_TEST_MODE" "Do request, host->testmode_timer = %x, host->testmode_tasklet = %x\n", &host->testmode_timer, &host->testmode_tasklet);
                mod_timer(&host->testmode_timer, jiffies + TESTMODE_TIMEOUT);
                host->testmode_timeout = 1;
//                pr_debug( "SDIO_TEST_MODE" "Trigger Timer[E], expires = %d, timeout = %d\n",host->testmode_timer.expires, host->testmode_timeout);
            }
            spin_unlock_irqrestore(&host->clk_gate_lock, flags); 
        }
    }
...
static void msdc_timer_testmode(unsigned long data)
{
    ...
    pr_debug( "SDIO_TEST_MODE" "Time out, start regular tuning, testmode_timeout = %d\n", host->testmode_timeout);
    pr_debug( "SDIO_TEST_MODE" "Time out, host->testmode_timer = %x, host->testmode_tasklet = %x\n", &host->testmode_timer, &host->testmode_tasklet);
