...
static kal_uint32 charging_hw_init(void *data)
{
    ...
    while(mt6333_get_rgs_power_on_ready() != 1)
    {
        ...
        {
            pr_debug("check rgs_power_on_ready fail\n");
            break;
        }
    }
    pr_debug("polling_time=%d of rgs_power_on_ready\n", polling_time);

#if defined(HIGH_BATTERY_VOLTAGE_SUPPORT)
    mt6333_set_rg_cv_sel(0);
    pr_debug("HIGH_BATTERY_VOLTAGE_SUPPORT\n");
#endif
...
static kal_uint32 charging_get_charging_status(void *data)
{
    ...
    if(ret_val == 0x1)
    {
        *(kal_uint32 *)data = KAL_TRUE;
        ...
        pr_debug("[MTK_SWCHR_SW_RECHR] Reg[0x04]=0x%x, Reg[0x19]=0x%x, Reg[0x2E]=0x%x, vbat_bal=%d, SWCHR_HW_RECHR_VOLTAGE=%d\n", 
...
static kal_uint32 charging_reset_watch_dog_timer(void *data)
{
    ...
    pr_debug("chr_en=%d, curr_vbat=%d, check_vbat=%d\n", val, vbat_bal, check_vbat);
#endif    
