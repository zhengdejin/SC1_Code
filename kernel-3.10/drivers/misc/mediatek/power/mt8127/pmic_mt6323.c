...
void PMIC_IMM_PollingAuxadcChannel(void)
{
	kal_uint32 ret=0;
	 ...
	 //pr_info(  "[PMIC_IMM_PollingAuxadcChannel] before:%d ",upmu_get_rg_adc_deci_gdly());
	...
	//pr_info(  "[PMIC_IMM_PollingAuxadcChannel] after:%d ",upmu_get_rg_adc_deci_gdly());
}
...
int PMIC_IMM_GetOneChannelValue(int dwChannel, int deCount, int trimd)
{
    ...
    */
	...
	mutex_unlock(&pmic_adc_mutex);
	    ...
	    switch(dwChannel){         
			...
			if( (count++) > count_time_out)
			{
                ...
                pr_info(  "[upmu_get_rg_adc_out_ch6] 0x%x\n", ret_data);
	            break;
	        case 2:    
    ...
    {
        //debug
        pr_info(  "[AUXADC] adc_result_temp=%d, adc_result=%d, r_val_temp=%d.\n", 
...
int pmic_get_buck_current(int avg_times)
{
    ...
    pr_info(  "[pmic_get_buck_current] raw_data=%d, val=%d, avg_times=%d\n", 
...
{
    pr_info(  "[upmu_interrupt_chrdet_int_en] val=%d.\r\n", val);
...
void pwrkey_int_handler(void)
{
    ...
    pr_info(  "[pwrkey_int_handler]....\n");
    
		if(upmu_get_pwrkey_deb()==1)    	    	
    {
        pr_info(  "[pwrkey_int_handler] Release pwrkey\n");
#if defined (CONFIG_MTK_KERNEL_POWER_OFF_CHARGING)
		if(g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT && timer_pre != 0)
		{
				...
				pr_info(  "[pmic_thread_kthread] timer_pos = %ld, timer_pre = %ld, timer_pos-timer_pre = %ld, long_pwrkey_press = %d\r\n",timer_pos, timer_pre, timer_pos-timer_pre, long_pwrkey_press);
				if(long_pwrkey_press)   //500ms
				{
					pr_info(  "[pmic_thread_kthread] Power Key Pressed during kernel power off charging, reboot OS\r\n");
					arch_reset(0, NULL);
				}
		}
#endif
    ...
    {
        pr_info(  "[pwrkey_int_handler] Press pwrkey\n");
#if defined (CONFIG_MTK_KERNEL_POWER_OFF_CHARGING)
...
void vbaton_undet_int_handler(void)
{
    ...
    pr_info(  "[vbaton_undet_int_handler]....\n");
...
void bvalid_det_int_handler(void)
{
    ...
    pr_info(  "[bvalid_det_int_handler]....\n");
...
void chrdet_int_handler(void)
{
    ...
    if (!upmu_get_rgs_chrdet())
    {
        ...
        {
            pr_info(  "[chrdet_int_handler] Unplug Charger/USB In Kernel Power Off Charging Mode!  Shutdown OS!\r\n");
            mt_power_off();
        }
    }
#endif
...
void fchr_key_int_handler(void)
{
    ...
    pr_info(  "[fchr_key_int_handler]....\n");
    
    if(upmu_get_fchrkey_deb()==1)
    {
        pr_info(  "[fchr_key_int_handler] Release fchrkey\n");
        kpd_pmic_rstkey_handler(0x0);
        upmu_set_rg_fchrkey_int_sel(0);
    }
    else
    {
        pr_info(  "[fchr_key_int_handler] Press fchrkey\n");
...
void accdet_int_handler(void)
{
    ...
    pr_info(  "[accdet_int_handler]....\n");

    ret = accdet_irq_handler();
    if(0 == ret){
        pr_info(  "[accdet_int_handler] don't finished\n");
    }
...
static int pmic_thread_kthread(void *x)
{
    ...
    pr_info(  "[pmic_thread_kthread] enter\n");

    /* Run on a process content */
    while (1) {
        mutex_lock(&pmic_mutex);
	
        pr_info(  "[pmic_thread_kthread] running\n");

        //--------------------------------------------------------------------------------
        ret=pmic_read_interface(INT_STATUS0,(&int_status_val_0),0xFFFF,0x0);
        pr_info(  "[INT] int_status_val_0=0x%x\n", int_status_val_0);
        ...
        pr_info(  "[INT] int_status_val_1=0x%x\n", int_status_val_1);
        ...
        ret=pmic_read_interface(INT_STATUS0,(&int_status_val_0),0xFFFF,0x0);
        pr_info(  "[INT] after ,int_status_val_0=0x%x\n", int_status_val_0);

        ret=pmic_read_interface(INT_STATUS1,(&int_status_val_1),0xFFFF,0x0);
        pr_info(  "[INT] after ,int_status_val_1=0x%x\n", int_status_val_1);
...
{
    pr_info(  "[mt6323_pmic_eint_irq] receive interrupt\n");
...
{
    pr_info(  "[PMIC_EINT_SETTING] start: CUST_EINT_MT6323_PMIC_NUM=%d\n",CUST_EINT_MT6323_PMIC_NUM);
    ...
    //pr_info(  "[CUST_EINT] GPIO37=GPIO_MODE_01 for EINT3 usage\n");
    ...
    pr_info(  "[CUST_EINT] CUST_EINT_MT6323_PMIC_NUM=%d\n", CUST_EINT_MT6323_PMIC_NUM);
    pr_info(  "[CUST_EINT] CUST_EINT_MT6323_PMIC_DEBOUNCE_CN=%d\n", CUST_EINT_MT6323_PMIC_DEBOUNCE_CN);
    //pr_info(  "[CUST_EINT] CUST_EINT_MT6323_PMIC_POLARITY=%d\n", CUST_EINT_MT6323_PMIC_POLARITY);
    //pr_info(  "[CUST_EINT] CUST_EINT_MT6323_PMIC_SENSITIVE=%d\n", CUST_EINT_MT6323_PMIC_SENSITIVE);
    pr_info(  "[CUST_EINT] CUST_EINT_MT6323_PMIC_TYPE=%d\n", CUST_EINT_MT6323_PMIC_TYPE);
    pr_info(  "[CUST_EINT] CUST_EINT_MT6323_PMIC_DEBOUNCE_EN=%d\n", CUST_EINT_MT6323_PMIC_DEBOUNCE_EN);
...
U32 pmic_read_interface (U32 RegNum, U32 *val, U32 MASK, U32 SHIFT)
{
    ...
    if(return_value!=0)
    {
        pr_info(  "[pmic_read_interface] Reg[%x]= pmic_wrap read data fail\n", RegNum);
    ...
    *val = (pmic6323_reg >> SHIFT);
    ...
    mutex_unlock(&pmic_access_mutex);
#else
    pr_info(  "[pmic_read_interface] Can not access HW PMIC\n");
#endif
...
U32 pmic_config_interface (U32 RegNum, U32 val, U32 MASK, U32 SHIFT)
{
    ...
    if(return_value!=0)
    {
        pr_info(  "[pmic_config_interface] Reg[%x]= pmic_wrap read data fail\n", RegNum);
        mutex_unlock(&pmic_access_mutex);
        return return_value;
    }
    ...
    if(return_value!=0)
    {
        pr_info(  "[pmic_config_interface] Reg[%x]= pmic_wrap write data fail\n", RegNum);
        mutex_unlock(&pmic_access_mutex);
        return return_value;
    }
    pr_debug(  "[pmic_config_interface] Reg[%x]=0x%x\n", RegNum, pmic6323_reg);
    ...
    mutex_unlock(&pmic_access_mutex);
#else
    pr_info(  "[pmic_config_interface] Can not access HW PMIC\n");
#endif
...
U32 pmic_read_interface_nolock (U32 RegNum, U32 *val, U32 MASK, U32 SHIFT)
{
    ...
    if(return_value!=0)
    {
        pr_info(  "[pmic_read_interface_nolock] Reg[%x]= pmic_wrap read data fail\n", RegNum);
        return return_value;
    }
...
#else
    pr_info(  "[pmic_read_interface_nolock] Can not access HW PMIC\n");
#endif
...
U32 pmic_config_interface_nolock (U32 RegNum, U32 val, U32 MASK, U32 SHIFT)
{
    ...
    if(return_value!=0)
    {
        pr_info(  "[pmic_config_interface_nolock] Reg[%x]= pmic_wrap read data fail\n", RegNum);
        return return_value;
    }
...
#else
    pr_info(  "[pmic_config_interface_nolock] Can not access HW PMIC\n");
#endif
...
static ssize_t store_pmic_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    U32 reg_value = 0;
    U32 reg_address = 0;
    pr_info(  "[store_pmic_access] \n");
    if(buf != NULL && size != 0)
    {
        pr_info(  "[store_pmic_access] buf is %s and size is %d \n",buf,size);
        reg_address = simple_strtoul(buf,&pvalue,16);

        if(size > 5)
        {
            reg_value = simple_strtoul((pvalue+1),NULL,16);
            pr_info(  "[store_pmic_access] write PMU reg 0x%x with value 0x%x !\n",reg_address,reg_value);
            ret=pmic_config_interface(reg_address, reg_value, 0xFFFF, 0x0);
        }
        else
        {
            ret=pmic_read_interface(reg_address, &g_reg_value, 0xFFFF, 0x0);
            pr_info(  "[store_pmic_access] read PMU reg 0x%x with value 0x%x !\n",reg_address,g_reg_value);
            pr_info(  "[store_pmic_access] Please use \"cat pmic_access\" to get value\r\n");
        }
    }
    return size;
}
...
{
    pr_info(  "[dct_pmic_VPROC_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VSYS_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VIO28_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VUSB_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VMCH_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VEMC_3V3_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VGP1_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VGP2_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VGP3_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VCN18_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VSIM1_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VSIM2_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VRTC_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VCAM_AF_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VIBR_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VRF18_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VIO18_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VCAMD_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VCAM_IO_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VTCXO_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VCAMA_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VCN28_enable_bt] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VCN28_enable_wifi] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VCN28_enable] %d\n", dctEnable);
...
{
    pr_info(  "****[dct_pmic_VMC_sel] value=%d \n", volt);
    ...
    if(volt == VOL_DEFAULT)     {upmu_set_rg_vmc_vosel(1);}
        ...
        pr_info(  "Error Setting %d. DO nothing.\r\n", volt);
    }
}

void dct_pmic_VMCH_sel(kal_uint32 volt)
{
    pr_info(  "****[dct_pmic_VMCH_sel] value=%d \n", volt);
...
{
    pr_info(  "****[dct_pmic_VEMC_3V3_sel] value=%d \n", volt);
...
{
    pr_info(  "****[dct_pmic_VGP1_sel] value=%d \n", volt);
...
{
    pr_info(  "****[dct_pmic_VGP2_sel] value=%d \n", volt);
...
{
    pr_info(  "****[dct_pmic_VGP3_sel] value=%d \n", volt);
...
{
    pr_info(  "****[dct_pmic_VCAM_AF_sel] value=%d \n", volt);
...
{
    pr_info(  "****[dct_pmic_VSIM1_sel] value=%d \n", volt);
...
{
    pr_info(  "****[dct_pmic_VSIM2_sel] value=%d \n", volt);
...
{
    pr_info(  "****[dct_pmic_VIBR_sel] value=%d \n", volt);
...
{
    pr_info(  "****[dct_pmic_VM_sel] value=%d \n", volt);
...
{
    pr_info(  "****[dct_pmic_VCAMD_sel] value=%d \n", volt);
...
{
    pr_info(  "****[dct_pmic_VCAMA_sel] value=%d \n", volt);
...
{
    pr_info(  "****[dct_pmic_VCN33_sel] value=%d \n", volt);
...
void pmic_ldo_enable(MT65XX_POWER powerId, kal_bool powerEnable)
{
    ...
    {
        pr_warn(  "[pmic_ldo_enable] UnKnown powerId (%d)\n", powerId);
    }
...
void pmic_ldo_vol_sel(MT65XX_POWER powerId, MT65XX_POWER_VOLTAGE powerVolt)
{
    ...
    if(     powerId == MT6323_POWER_LDO_VIO28)      { pr_info(  "[dct_pmic_VIO28_enable] No vlotage can setting!\n"); }
    else if(powerId == MT6323_POWER_LDO_VUSB)       { pr_info(  "[dct_pmic_VUSB_enable] No vlotage can setting!\n"); }
    ...
    else if(powerId == MT6323_POWER_LDO_VCN_1V8)        {pr_info(  "[dct_pmic_VCN_1V8_enable] No vlotage can setting!\n");}
    ...
    else if(powerId == MT6323_POWER_LDO_VRTC)        { pr_info(  "[dct_pmic_VRTC_enable] No vlotage can setting!\n");}
    ...
    else if(powerId == MT6323_POWER_LDO_VRF18)        { pr_info(  "[dct_pmic_VRF18_enable] No vlotage can setting!\n");}
    else if(powerId == MT6323_POWER_LDO_VIO18)        { pr_info(  "[dct_pmic_VIO18_enable] No vlotage can setting!\n");}
    else if(powerId == MT6323_POWER_LDO_VCAMD)        { dct_pmic_VCAMD_sel(powerVolt); }
    else if(powerId == MT6323_POWER_LDO_VCAM_IO)    { pr_info(  "[dct_pmic_VCAM_IO_enable] No vlotage can setting!\n");}
    else if(powerId == MT6323_POWER_LDO_VTCXO)        { pr_info(  "[dct_pmic_VTCXO_enable] No vlotage can setting!\n");}
    else if(powerId == MT6323_POWER_LDO_VA)         { pr_info(  "[dct_pmic_VA_enable] No vlotage can setting!\n"); }
    ...
    else if(powerId == MT6323_POWER_LDO_VCN28)      { pr_info(  "[dct_pmic_VCN28_enable] No vlotage can setting!\n"); }
    else
    {
        pr_warn(  "[pmic_ldo_ldo_vol_sel] UnKnown powerId (%d)\n", powerId);
    }
...
void pmic_debug_init(void)
{
    ...
    if (!mt_pmic_dir) {
        pr_info(  "fail to mkdir /proc/mt_pmic\n" );
        return;
    }
    ...
    if (!entry) {
        pr_info(  "fail to create entry /proc/mt_pmic/dump_ldo_status\n" );
        return;
    }
    #else
...
void PMIC_INIT_SETTING_V1(void)
{
    ...
    {
        pr_info(  "[Kernel_PMIC_INIT_SETTING_V1] PMIC Chip = %x\n",chip_version);
        pr_info(  "[Kernel_PMIC_INIT_SETTING_V1] 20130604_0.85v\n");
...
ret = pmic_config_interface(0x778,0x1,0x1,15); // [15:15]: RG_VREF18_ENB_MD; 
    ...
    {
        pr_info(  "[Kernel_PMIC_INIT_SETTING_V1] PMIC Chip = %x\n",chip_version);
        pr_info(  "[Kernel_PMIC_INIT_SETTING_V1] 20130328_0.85v\n");
...
ret = pmic_config_interface(0x778,0x1,0x1,15); // [15:15]: RG_VREF18_ENB_MD; 
    ...
    {
        pr_info(  "[Kernel_PMIC_INIT_SETTING_V1] Unknown PMIC Chip (%x)\n",chip_version);
    }
...
void pmic_setting_depends_rtc(void)
{
    ...
    if( crystal_exist_status() )
    {
        ...
        pr_info(  "[pmic_setting_depends_rtc] With 32K. Reg[0x%x]=0x%x\n", 
    ...
    else
    {
        ...
        pr_info(  "[pmic_setting_depends_rtc] Without 32K. Reg[0x%x]=0x%x\n", 
            ANALDO_CON1, upmu_get_reg_value(ANALDO_CON1));
    }
#else
    pr_info(  "[pmic_setting_depends_rtc] no define CONFIG_MTK_RTC\n");
#endif
}
...
static int pmic_mt6323_probe(struct platform_device *dev)
{
    ...
    pr_info(  "******** MT6323 pmic driver probe!! ********\n" );


    //get PMIC CID
    ret_val=upmu_get_cid();
    pr_info(  "MT6323 PMIC CID=0x%x\n", ret_val );
    ...
    pr_info(  "[PMIC_INIT_SETTING_V1] Done\n");
    PMIC_CUSTOM_SETTING_V1();
    pr_info(  "[PMIC_CUSTOM_SETTING_V1] Done\n");
    ...
    //PMIC Interrupt Service
    PMIC_EINT_SETTING();
    pr_info(  "[PMIC_EINT_SETTING] Done\n");
    
    kthread_run(pmic_thread_kthread, NULL, "pmic_thread_kthread");
    pr_info(  "[pmic_thread_kthread] Done\n");
    ...
    pmic_debug_init();
    pr_info(  "[PMIC] pmic_debug_init : done.\n" );
    ...
    pr_info(  "[PMIC] VTCXO control by SRCLKEM due to MTK_ENABLE_MD5, Reg[0x402]=0x%x\n",
...
{
    pr_info(  "******** MT6323 pmic driver remove!! ********\n" );
...
{
    pr_info(  "******** MT6323 pmic driver resume!! ********\n" );
...
{
	pr_info(  "******** MT6323 pmic driver early suspend!! ********\n" );
	upmu_set_rg_vref18_enb(0);
	//upmu_set_rg_adc_deci_gdly(0);
	upmu_set_rg_clksq_en_aux(1);
	upmu_set_rg_aud26m_div4_ck_pdn(0);
...
{
	pr_info(  "******** MT6323 pmic driver early resume!! ********\n" );
	upmu_set_rg_vref18_enb(0);
	//upmu_set_rg_adc_deci_gdly(0);
	upmu_set_rg_clksq_en_aux(1);
	upmu_set_rg_aud26m_div4_ck_pdn(0);
...
static int __init pmic_mt6323_init(void)
{
    ...
    if (ret) {
        pr_info(  "****[pmic_mt6323_init] Unable to device register(%d)\n", ret);
        return ret;
    }
    ret = platform_driver_register(&pmic_mt6323_driver);
    if (ret) {
        pr_info(  "****[pmic_mt6323_init] Unable to register driver (%d)\n", ret);
        return ret;
    }
    ...
    pr_info(  "****[pmic_mt6323_init] Initialization : DONE !!\n");
