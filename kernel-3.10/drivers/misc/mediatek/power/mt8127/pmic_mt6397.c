...
int PMIC_IMM_GetOneChannelValue(int dwChannel, int deCount, int trimd)
{
    ...
    */
    ...
    do
    {
        ...
        switch(dwChannel){         
            case 0:    
                ...
                {    
                    ...
                    {
                        pr_debug(  "[IMM_GetOneChannelValue_PMIC] (%d) Time out!\n", dwChannel);
                        break;
                    }
                }
                if(trimd == 1) {
        ...
        {
            //debug
            pr_debug(  "[AUXADC] u4Sample_times=%d, ret_data=%d, u4channel[%d]=%d.\n", 
...
void do_chrdet_int_task(void)
{
    ...
    if(ret==1)
    {
        ...
        #if defined(CONFIG_POWER_EXT)
        mt_usb_connect();
        pr_info(  "[do_chrdet_int_task] call mt_usb_connect() in EVB\n");
        #endif
    }
    else
    {
		...
		{
			pr_info(  "[pmic_thread_kthread] Unplug Charger/USB In Kernel Power Off Charging Mode!  Shutdown OS!\r\n");
			mt_power_off();
		}
#endif
        ...
        #if defined(CONFIG_POWER_EXT)
        mt_usb_disconnect();
        pr_info(  "[do_chrdet_int_task] call mt_usb_disconnect() in EVB\n");
        #endif
    }
    
    #if defined(CONFIG_POWER_EXT)
    pr_info(  "[do_chrdet_int_task] Environment=FPGA/EVB\n");
    #else
    ...
    if(g_bat_init_flag==1)
        ...
        pr_info(  "[do_chrdet_int_task] battery thread not ready, will do after bettery init.\n");    
    #endif
}
#else
...
void ov_int_handler(void)
{
    ...
    pr_info(  "[ov_int_handler]....\n");
}
void ldo_int_handler(void)
{
    kal_uint32 ret=0;

    pr_info(  "[ldo_int_handler]....\n");
...
void homekey_int_handler(void)
{
    ...
    //pr_info(  "[homekey_int_handler]....\n");

    if(upmu_get_homekey_deb()==1)
    {
        pr_info(  "[homekey_int_handler] Release HomeKey\r\n");
        kpd_pmic_rstkey_handler(0x0);
    }
    else
    {
        pr_info(  "[homekey_int_handler] Press HomeKey\r\n");
...
void pwrkey_rstb_int_handler(void)
{
    ...
    pr_info(  "[pwrkey_rstb_int_handler]....\n");
...
#ifndef CONFIG_MT8127_FPGA
    //to do, check EINT setting in dct tool
    pr_info(  "[PMIC_EINT_SETTING] start: CUST_EINT_MT6397_PMIC_NUM=%d\n",CUST_EINT_MT6323_PMIC_NUM);
#endif
    //ON/OFF interrupt
    cust_pmic_interrupt_en_setting();
...
//to do, check EINT setting in dct tool     
    ...
    pr_info(  "[CUST_EINT] CUST_EINT_MT6397_PMIC_NUM=%d\n", CUST_EINT_MT6323_PMIC_NUM);
    pr_info(  "[CUST_EINT] CUST_EINT_MT6397_PMIC_DEBOUNCE_CN=%d\n", CUST_EINT_MT6323_PMIC_DEBOUNCE_CN);
    pr_info(  "[CUST_EINT] CUST_EINT_MT6397_PMIC_TYPE=%d\n", CUST_EINT_MT6323_PMIC_TYPE);    
    pr_info(  "[CUST_EINT] CUST_EINT_MT6397_PMIC_DEBOUNCE_EN=%d\n", CUST_EINT_MT6323_PMIC_DEBOUNCE_EN);
#endif    
}
...
void dct_pmic_VIO28_enable(kal_bool dctEnable)
{
    ...
    pr_info(  "[dct_pmic_VIO28_enable] No enable can setting!\n");
#else
...
{
    pr_info(  "[dct_pmic_VCAMIO_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VCAMAF_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VGP4_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VGP5_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VGP6_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VA28_enable] %d\n", dctEnable);
...
{
    pr_info(  "[dct_pmic_VIO28_enable] No voltage can setting!\n");
}

void dct_pmic_VUSB_sel(kal_uint32 volt)
{
    pr_info(  "[dct_pmic_VUSB_sel] No voltage can setting!\n");
}
...
{
    pr_info(  "****[dct_pmic_VCAMIO_sel] value=%d \n", volt);
...
{
    pr_info(  "****[dct_pmic_VCAMAF_sel] value=%d \n", volt);
...
{
    pr_info(  "****[dct_pmic_VGP4_sel] value=%d \n", volt);
...
{
    pr_info(  "****[dct_pmic_VGP5_sel] value=%d \n", volt);
...
{
    pr_info(  "****[dct_pmic_VGP6_sel] value=%d \n", volt);
...
{
    pr_info(  "[dct_pmic_VRTC_sel] No voltage can setting!\n");
}

void dct_pmic_VTCXO_sel(kal_uint32 volt)
{
    pr_info(  "[dct_pmic_VTCXO_sel] No voltage can setting!\n");
}

void dct_pmic_VA28_sel(kal_uint32 volt)
{
    pr_info(  "[dct_pmic_VA28_sel] No voltage can setting!\n");
}
...
static ssize_t show_BUCK_VPCA7_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    pr_debug(  "[EM] BUCK_VPCA7_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_BUCK_VSRMCA7_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    */
    ...
    pr_debug(  "[EM] BUCK_VSRMCA7_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_BUCK_VCA15_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    pr_debug(  "[EM] BUCK_VCA15_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_BUCK_VSRMCA15_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    */
    ...
    pr_debug(  "[EM] BUCK_VSRMCA15_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_BUCK_VCORE_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    */
    ...
    pr_debug(  "[EM] BUCK_VCORE_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_BUCK_VDRM_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    */
    ...
    pr_debug(  "[EM] BUCK_VDRM_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VCAMIO_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    */
    ...
    pr_debug(  "[EM] LDO_VCAMIO_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VCAMAF_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    */
    ...
    pr_debug(  "[EM] LDO_VCAMAF_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_BUCK_VPCA7_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    pr_debug(  "[EM] BUCK_VPCA7_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_BUCK_VSRMCA7_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    pr_debug(  "[EM] BUCK_VSRMCA7_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_BUCK_VCA15_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    pr_debug(  "[EM] BUCK_VCA15_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_BUCK_VSRMCA15_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    pr_debug(  "[EM] BUCK_VSRMCA15_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_BUCK_VCORE_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    pr_debug(  "[EM] BUCK_VCORE_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_BUCK_VDRM_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    pr_debug(  "[EM] BUCK_VDRM_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VUSB_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    pr_debug(  "[EM] LDO_VUSB_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VCAMIO_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    pr_debug(  "[EM] LDO_VCAMIO_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VCAMAF_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    pr_debug(  "[EM] LDO_VCAMAF_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VGP5_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    pr_debug(  "[EM] LDO_VGP5_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VGP6_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    pr_debug(  "[EM] LDO_VGP6_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VTCXO_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    pr_debug(  "[EM] LDO_VTCXO_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VA28_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    pr_debug(  "[EM] LDO_VA28_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
void PMIC_INIT_SETTING_V1(void)
{
    ...
    if(chip_version >= PMIC6397_E1_CID_CODE)
    {
        ...
        #if 0
        //dump register
        pr_info(  "[pmic_init_setting] Reg[0x%x]=0x%x\n", 0x002, upmu_get_reg_value(0x002));
...
void pmic_low_power_setting(void)
{
    ...
    chip_version = upmu_get_cid();
    pr_info(  "[pmic_low_power_setting] CLKCTL - 20121018 by Juinn-Ting\n");
    ...
    pr_info(  "[pmic_low_power_setting] Done\n");
}
...
{
    pr_info(  "[get_spm_gpu_status] wait spm driver service ready\n");
...
void pmic_gpu_power_enable(int power_en)
{
    ...
    {
        pr_info(  "[pmic_gpu_power_enable] gpu is not powered by VRF18_2\n");
    }
    else
    {
        if(power_en == 1)
        ...
        pr_info(  "[pmic_gpu_power_enable] Reg[0x%x]=%x\n", 
...
static int pmic_mt6397_probe(struct platform_device *dev)
{
    ...
    pr_info(  "[pmic_thread_kthread] add for EVB\n");
    #endif
...
{
    pr_info(  "******** MT6397 pmic driver remove!! ********\n" );
...
{
    pr_info(  "******** MT6397 pmic driver shutdown!! ********\n" );
}
...
static int pmic_mt6397_suspend(struct platform_device *dev, pm_message_t state)
{
    ...
    //pr_info(  "******** MT6397 pmic driver suspend!! ********\n" );
#if 0 //MT6320 config
...
static int pmic_mt6397_resume(struct platform_device *dev)
{
    ...
    //pr_info(  "******** MT6397 pmic driver resume!! ********\n" );
...
static int __init pmic_mt6397_init(void)
{
    ...
    if (ret) {
        pr_info(  "****[pmic_mt6397_init] Unable to device register(%d)\n", ret);
        return ret;
    }
    ret = platform_driver_register(&pmic_mt6397_driver);
    if (ret) {
        pr_info(  "****[pmic_mt6397_init] Unable to register driver (%d)\n", ret);
        return ret;
    }
    ...
    pr_info(  "****[pmic_mt6397_init] Initialization : DONE !!\n");
