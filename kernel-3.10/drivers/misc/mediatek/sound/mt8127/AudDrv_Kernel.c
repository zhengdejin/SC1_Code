...
void CheckHDMIWriteWaitEvent(void)
{
    ...
    {
        pr_err("HDMI Audio Dump FTrace, OverTimeCounter=%d n", OverTimeCounter);
...
static long AudDrv_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        case SET_HDMI_CLOCK_SOURCE:
        {
            ...
            pr_info("SET_HDMI_CLOCK_SOURCE ClkApllSel:%d,SampleRate:%d\n",
        ...
        case SET_AUDIO_STATE:
        {
            ...
            dmb();
            pr_info("SET_AUDIO_STATE bBgsFlag:%d,bRecordFlag:%d,bSpeechFlag:%d,bTtyFlag:%d,bVT:%d,bAudio:%d \n",
        ...
        case GET_AUDIO_STATE:
        {
            ...
            pr_info("GET_AUDIO_STATE bBgsFlag:%d,bRecordFlag:%d,bSpeechFlag:%d,bTtyFlag:%d,bVT:%d,bAudio:%d \n",
                        SPH_Ctrl_State.bBgsFlag,
                        SPH_Ctrl_State.bRecordFlag,
                        SPH_Ctrl_State.bSpeechFlag,
                        SPH_Ctrl_State.bTtyFlag,
...
ssize_t AudDrv_MEMIF_Read(struct file *fp,  char __user *data, size_t count, loff_t *offset)
{
    ...
    while (count)
    {
        ...
        else
        {
            ...
            {
                /*
                pr_info("AudDrv_AWB_Read 3, read_size2:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
            ...
            {
                /*
                pr_err("AudDrv_MEMIF_Read Fail 3 copy to user Read_Data_Ptr:%p, pucVirtBufAddr:%p, u4DMAReadIdx:0x%x, DMA_Read_Ptr:0x%x, read_size:%x",
                    Read_Data_Ptr, Afe_Block->pucVirtBufAddr, Afe_Block->u4DMAReadIdx, DMA_Read_Ptr, read_size);*/
                if (read_count == 0)
                {
                    return -1;
...
static int AudDrv_remap_mmap(struct file *flip, struct vm_area_struct *vma)
{
    ...
    {
        pr_err("AudDrv_remap_mmap remap_pfn_range Fail \n");
        return -EIO;
    }
