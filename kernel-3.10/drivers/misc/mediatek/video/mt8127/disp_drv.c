...
#if 1
	/* workaround for lk not match with kerenl, must before DAL_Init*/
	/* 1. lk base > kernel base*/
	/*     when update from KK to L*/
	/* 2. lk base >> kernel base*/
	/*     when kernel only 512M but lk 1G or large*/
	extern struct {u32 base; u32 size;} bl_fb;
	if (bl_fb.base > fbPA) {
		extern int m4u_dma_cache_flush_all();
		unsigned int lk_map_va;

		pr_warn("lk fb base: 0x%x != kernel fb base: 0x%x\n", bl_fb.base, fbPA);
		memset(fbVA, 0, DISP_GetFBRamSize());
		if ((fbPA + DISP_GetVRamSize()) != (bl_fb.base + bl_fb.size)) {
			lk_map_va = (unsigned int)ioremap_nocache(bl_fb.base, bl_fb.size);
			memcpy(fbVA, lk_map_va, DISP_GetFBRamSize());
		} else
			memcpy(fbVA, fbVA + DISP_GetVRamSize() - bl_fb.size, DISP_GetFBRamSize());
		m4u_dma_cache_flush_all();

		DISP_WaitVSYNC();
		disp_path_get_mutex();
		disp_path_config_layer_addr(FB_LAYER-1, fbPA);
		disp_path_config_layer_addr(FB_LAYER, fbPA);
