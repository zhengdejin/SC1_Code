...
#endif
{
    ...
    case MTKFB_POWEROFF:
    {
		...
		if (wait_event_interruptible_timeout(_dsi_wait_vm_done_queue, !_isdsienginebusy(), HZ/10) == 0)
...
static void mtkfb_fb_565_to_8888(struct fb_info *fb_info)
{
    ...
    PRNERR("mtkfb_fb_565_to_8888 xres=%d yres=%d fbsize=0x%X x_virtual=%d s=%p d=%p\n",
